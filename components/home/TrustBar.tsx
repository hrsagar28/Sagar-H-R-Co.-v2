  ...
  return (
    <section className="py-12 bg-white border-b border-brand-border/60 overflow-hidden">
      <div className="container mx-auto max-w-7xl px-4 md:px-6">
        <Reveal width="100%">
          <p className="text-center text-xs font-bold uppercase tracking-widest text-brand-dark mb-8">
          ...
          <div className="flex flex-wrap justify-center gap-4 md:gap-16 opacity-60 grayscale hover:grayscale-0 transition-all duration-500">
            {INDUSTRIES.map((item, idx) => (
              <div key={idx} className="flex items-center gap-3 group">
                ...
                <span className="font-heading font-bold text-brand-dark text-xs md:text-sm">{item.title}</span>
              </div>
            ))}
          </div>
        </Reveal>
