...
const ChaosToOrder: React.FC = () => {
  ...
  return (
    <section
      className="relative overflow-hidden bg-brand-bg py-20 text-[#0a0908] md:py-28"
      aria-labelledby="chaos-to-order-title"
    >
      {/* Soft paper tone & faint vertical brass rule on the left margin */}
      <div className="pointer-events-none absolute inset-y-0 left-0 hidden w-[2px] bg-gradient-to-b from-transparent via-[#b8924c]/25 to-transparent md:block" />
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <Reveal variant="fade-up" delay={0.08}>
            ...
            >
              Messy books, in.
              <br />
              <span className="font-serif font-normal italic text-[#8b3a2f]">Clean books, out.</span>
            </h2>
          </Reveal>
          <Reveal variant="fade-up" delay={0.18}>
            <p
              className="max-w-md font-medium leading-relaxed text-[#2A251D]/75 md:text-right"
      ...
      <div className="container mx-auto max-w-7xl px-4 md:px-6">
        <Reveal variant="fade-up" delay={0.1} width="100%">
          <div
            ...
            className="relative aspect-[3/4] w-full cursor-ew-resize touch-pan-y select-none overflow-hidden rounded-[1.75rem] border border-[#0a0908]/10 bg-[#eae5d9] shadow-[0_30px_60px_-20px_rgba(10,9,8,0.25)] sm:aspect-[4/3] md:rounded-[2rem] lg:aspect-[16/10]"
            role="group"
          ...
          >
            ...
            {/* Divider line */}
            <div
              className="pointer-events-none absolute bottom-0 top-0 w-px bg-[#0a0908]/40"
            ...
            <button
              ...
              className={`absolute top-1/2 flex h-12 w-12 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-[#f4f1ea]/10 bg-[#0a0908] text-[#f4f1ea] shadow-lg shadow-[#0a0908]/30 transition-transform duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#b8924c] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f4f1ea] md:h-14 md:w-14 ${
            ...
            <div className="pointer-events-none absolute left-3 top-3 md:left-6 md:top-5" style={{ zIndex: 3 }}>
              <span
                className="inline-block rounded bg-[#8b3a2f]/85 px-2 py-1 font-mono uppercase tracking-[0.15em] text-[#f4f1ea] backdrop-blur-sm"
            ...
            >
              <span
                className="inline-block rounded bg-[#0a0908]/85 px-2 py-1 font-mono uppercase tracking-[0.15em] text-[#f4f1ea] backdrop-blur-sm"
                style={{ fontSize: 'clamp(9px, 1.4vw, 11px)' }}
              >
        ...
        <Reveal variant="fade-up" delay={0.2}>
          <p className="mt-5 text-center font-mono text-[11px] uppercase tracking-[0.2em] text-[#0a0908]/55 md:mt-7">
...
const ChaosPane: React.FC = () => {
  return (
    <div className="absolute inset-0 overflow-hidden">
      ...
      >
        <div className="flex h-full items-center gap-3 whitespace-nowrap px-3 font-mono text-[11px] text-[#0a0908]/65">
          <span>18,240.00</span>
          <span>+</span>
          <span>4,610.00</span>
          <span>+</span>
      ...
      >
        <div
          className="h-full w-full rounded-full border-[3px] border-[#0a0908]/35"
...
const DocCard: React.FC<ChaosDoc & { seed: number }> = ({
  ...
  return (
    ...
    >
      <div
        className="border-[#0a0908]/8 relative h-full w-full overflow-hidden rounded-[2px] border shadow-[0_6px_14px_-6px_rgba(10,9,8,0.35)]"
...
const InvoiceBody: React.FC<{ seed: number }> = ({ seed }) => {
  ...
  return (
    <>
      <div className="flex items-center justify-between border-b border-dashed border-[#0a0908]/15 px-3 pb-1.5 pt-2">
        <span className="font-mono text-[8px] uppercase tracking-[0.2em] text-[#0a0908]/65 md:text-[9px]">
          Invoice · {String(1024 + seed * 37).padStart(4, '0')}
        </span>
        <span className="font-mono text-[9px] text-[#0a0908]/45 md:text-[8px]">
      ...
      <div className="space-y-1.5 p-3">
        {Array.from({ length: lines }).map((_, i) => (
          <div
            key={i}
            className="h-[2px] rounded-full bg-[#0a0908]/15"
            style={{ width: `${55 + ((seed * 13 + i * 19) % 40)}%` }}
          />
        ))}
        <div className="flex justify-between pt-1.5">
          <div className="h-[3px] w-[30%] rounded-full bg-[#0a0908]/25" />
          <div className="h-[3px] w-[22%] rounded-full bg-[#0a0908]/25" />
        </div>
      </div>
    </>
  );
...
const GSTRBody: React.FC<{ seed: number }> = ({ seed }) => {
  ...
  return (
    <>
      <div
        className="flex items-center justify-between border-b border-[#8b3a2f]/35 px-3 pb-1.5 pt-2"
        style={{ backgroundColor: 'rgba(139,58,47,0.06)' }}
      >
        <span className="font-mono text-[8px] font-bold uppercase tracking-[0.2em] text-[#8b3a2f] md:text-[9px]">
          GSTR-{seed % 2 === 0 ? '3B' : '1'} · {seed % 2 === 0 ? 'SUMMARY' : 'OUTWARD SUPPLY'}
        </span>
        <span className="font-mono text-[9px] text-[#0a0908]/55 md:text-[8px]">
          {['MAR-24', 'APR-24', 'MAY-24', 'JUN-24', 'JUL-24', 'AUG-24'][seed % 6]}
        </span>
      </div>
      <div className="px-3 py-2">
        <div className="grid grid-cols-[1fr_1.2fr_1fr] gap-1 border-b border-[#0a0908]/10 pb-1 font-mono text-[8px] uppercase tracking-[0.1em] text-[#0a0908]/50 md:text-[7px]">
          <span>HSN</span>
        ...
        {rows.map((r, i) => (
          <div
            key={i}
            className="grid grid-cols-[1fr_1.2fr_1fr] gap-1 border-b border-[#0a0908]/5 py-[3px] font-mono text-[9px] tabular-nums text-[#0a0908]/80 last:border-b-0 md:text-[8px]"
          >
            <span>{r.hsn}</span>
        ...
        <div className="mt-1 flex items-center justify-between border-t border-[#8b3a2f]/40 pt-1.5">
          <span className="font-mono text-[9px] font-bold uppercase text-[#8b3a2f] md:text-[8px]">Total tax</span>
          <span className="font-mono text-[9px] font-bold tabular-nums text-[#8b3a2f] md:text-[8px]">72,071</span>
        </div>
      </div>
    </>
  );
...
const BRSBody: React.FC<{ seed: number }> = ({ seed }) => {
  return (
    <>
      <div className="flex items-center justify-between border-b border-dashed border-[#0a0908]/15 px-3 pb-1.5 pt-2">
        <span className="font-mono text-[8px] font-bold uppercase tracking-[0.2em] text-[#0a0908]/65 md:text-[9px]">
          Bank Reconciliation
        </span>
        <span className="font-mono text-[9px] text-[#0a0908]/45 md:text-[8px]">
      ...
      <div className="px-3 py-2 font-mono text-[9px] tabular-nums text-[#0a0908]/80 md:text-[8px]">
        ...
        <div className="my-1.5 h-px bg-[#0a0908]/15" />
        ...
        >
          <span className="font-bold uppercase tracking-[0.1em] text-[#8b3a2f]">Difference</span>
          <span className="font-bold text-[#8b3a2f]">7,940</span>
        </div>
      </div>
    </>
  );
...
const ITRBody: React.FC<{ seed: number }> = ({ seed }) => {
  return (
    <>
      <div
        className="border-b-2 border-[#0a0908]/30 px-3 pb-1.5 pt-2"
        style={{ backgroundColor: 'rgba(184,146,76,0.12)' }}
      >
        <div className="flex items-center justify-between">
          <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-[#0a0908]/55 md:text-[8px]">
            Form ITR-{[1, 2, 3, 4, 5][seed % 5]}
          </span>
          <span className="font-mono text-[8px] uppercase tracking-[0.15em] text-[#0a0908]/45 md:text-[7px]">
            AY {['2023-24', '2024-25', '2025-26'][seed % 3]}
          </span>
        </div>
        <div className="mt-[2px] font-mono text-[8px] font-bold uppercase tracking-[0.15em] text-[#0a0908] md:text-[9px]">
          Income Tax Return
        </div>
      </div>
      <div className="space-y-[3px] px-3 py-2 font-mono text-[9px] text-[#0a0908]/80 md:text-[8px]">
...
const TDSBody: React.FC<{ seed: number }> = ({ seed }) => {
  return (
    <>
      <div className="border-b border-[#0a0908]/20 px-3 pb-1.5 pt-2">
        <div className="flex items-center justify-between">
          <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-[#0a0908]/55 md:text-[8px]">
            Form 16{seed % 2 ? 'A' : ''}
          </span>
          <span className="font-mono text-[9px] text-[#0a0908]/45 md:text-[8px]">Q{(seed % 4) + 1} · FY 24-25</span>
        </div>
        <div className="mt-[2px] font-mono text-[8px] font-bold uppercase tracking-[0.1em] text-[#0a0908] md:text-[9px]">
          Certificate of TDS
        </div>
      </div>
      <div className="space-y-[3px] px-3 py-2 font-mono text-[9px] tabular-nums text-[#0a0908]/80 md:text-[8px]">
        ...
        <div className="flex justify-between border-t border-[#0a0908]/15 pt-[3px] font-bold">
...
const ChequeBody: React.FC<{ seed: number }> = ({ seed }) => {
  return (
    <div className="relative h-full w-full px-3 py-2 font-mono text-[9px] text-[#0a0908]/80 md:text-[8px]">
      <div className="flex items-start justify-between">
        <div>
          <div className="text-[8px] font-bold uppercase tracking-[0.15em] text-[#0a0908] md:text-[9px]">HDFC Bank</div>
          <div className="text-[8px] text-[#0a0908]/55 md:text-[7px]">Devaraja Urs Road, Mysuru</div>
        </div>
        <div className="text-[8px] tabular-nums text-[#0a0908]/55 md:text-[7px]">
      ...
      <div className="mt-1.5 flex items-center justify-between">
        <span className="text-[8px] text-[#0a0908]/55 md:text-[7px]">Rupees _________________________</span>
        <div className="rounded-sm border border-[#0a0908]/40 px-2 py-[1px] font-bold tabular-nums">₹ 42,500</div>
      </div>
      <div className="absolute bottom-[22%] left-3 right-3 text-right text-[9px] italic text-[#0a0908]/35 md:text-[8px]">
        signature
      </div>
      {/* MICR band */}
      <div
        className="absolute bottom-0 left-0 right-0 border-t border-[#0a0908]/15 px-3 py-1 font-mono text-[9px] tracking-[0.2em] text-[#0a0908]/70 md:text-[8px]"
  ...
  return (
    <div className="absolute inset-0 overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4 md:p-10">
        <div
          className="w-full max-w-[520px] border border-[#b8924c]/50 bg-[#f8f5ec] shadow-[0_20px_40px_-18px_rgba(10,9,8,0.25)]"
          style={{ borderRadius: '4px' }}
        >
          {/* Masthead */}
          <div className="border-b border-[#b8924c]/40 px-5 pb-3 pt-5 md:px-7 md:pb-4 md:pt-7">
            <div className="flex items-baseline justify-between">
              <span className="font-mono text-[9px] uppercase tracking-[0.3em] text-[#8b3a2f] md:text-[10px]">
                Statement of Profit &amp; Loss
              </span>
              <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-[#0a0908]/55 md:text-[10px]">
                FY 2025&ndash;26
              </span>
            </div>
            <div className="mt-1 font-serif text-[11px] italic text-[#0a0908]/70 md:text-xs">
          ...
          <div className="px-5 py-4 md:px-7 md:py-5">
            <div className="font-mono text-[11px] text-[#0a0908] md:text-[13px]">
              {LEDGER_ROWS.map((row, i) => (
                <div key={i}>
                  {row.section && <div className="my-2 h-px bg-[#0a0908]/10" />}
                  <div
          ...
          {/* Footer — closed seal */}
          <div className="flex items-center justify-between border-t border-[#b8924c]/30 px-5 pb-5 pt-2 md:px-7 md:pb-6">
            <span className="font-mono text-[9px] uppercase tracking-[0.25em] text-[#0a0908]/55 md:text-[10px]">
              Reconciled · Audited
            </span>
            <ClosedSeal />
          </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center leading-none">
        <span className="font-serif text-[10px] italic text-[#8b3a2f]">closed</span>
        <span className="mt-[2px] font-mono text-[8px] tracking-[0.15em] text-[#0a0908]/70">FY25-26</span>
      </div>
    </div>
  );
};
