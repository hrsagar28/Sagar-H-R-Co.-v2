...
const ScrollyTelling: React.FC<ScrollyTellingProps> = ({ items, className = '' }) => {
  ...
  useEffect(() => {
    ...
    scheduleMeasure(); // Initial measurement
  ...
  return (
    <div className={`relative z-20 w-full bg-brand-black text-white ${className}`}>
      {/* --- MOBILE LAYOUT (Vertical Stack) --- */}
      <div className="flex flex-col gap-20 px-4 py-20 md:hidden">
        {items.map((item, i) => (
          <div key={item.id} className="flex flex-col gap-8">
            <Reveal variant="scale" width="100%">
              <div className="relative aspect-square w-full overflow-hidden rounded-[2rem] border border-white/10 bg-brand-dark/50 shadow-2xl">
                {/* Visual Wrapper */}
                <div className="absolute inset-0 flex items-center justify-center p-4">
                  <div className="flex h-full w-full origin-center scale-75 items-center justify-center">
                    {item.visual}
                  </div>
                </div>
                {/* Number Watermark */}
                <span className="absolute right-6 top-4 select-none font-heading text-6xl font-bold leading-none text-white/5">
                  0{i + 1}
                </span>
              </div>
            </Reveal>
            ...
            <Reveal variant="fade-up">
              <div className="px-2">
                <h2 className="mb-4 font-heading text-3xl font-bold text-white">{item.title}</h2>
                <p className="text-lg font-medium leading-relaxed text-zinc-400">{item.description}</p>
              </div>
            </Reveal>
          </div>
        ))}
      ...
      >
        <div className="sticky top-0 flex h-screen w-full flex-row overflow-hidden bg-brand-black">
          {/* PROGRESS BAR (Left Edge) */}
          <div className="absolute left-10 top-1/2 z-30 h-64 w-[2px] -translate-y-1/2 rounded-full bg-white/10">
            <div
              className="w-full rounded-full bg-[#4ADE80] shadow-[0_0_15px_#4ADE80] transition-all duration-100 ease-linear"
              style={{
          ...
          {/* LEFT: Text Content */}
          <div className="relative z-20 flex h-full w-1/2 items-center justify-center p-20 pl-32">
            <div className="relative w-full max-w-lg">
              {items.map((item, i) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="absolute left-0 top-1/2 w-full -translate-y-1/2 transform transition-opacity duration-100 will-change-transform"
                    style={{
                      opacity: opacity,
                      filter: `blur(${blur}px)`,
                  ...
                  >
                    ...
                    <span className="absolute -left-16 -top-24 -z-10 select-none font-heading text-[12rem] font-bold leading-none text-white/5">
                      0{i + 1}
                    </span>

                    <h2 className="mb-8 font-heading text-5xl font-bold leading-none tracking-tight text-white drop-shadow-xl lg:text-7xl">
                      {item.title}
                    </h2>
                    <p className="max-w-md border-l-2 border-[#4ADE80] pl-6 text-xl font-medium leading-relaxed text-zinc-300">
                      {item.description}
                    </p>
                  </div>
                );
          ...
          <div className="perspective-[1000px] relative flex h-full w-1/2 items-center justify-center overflow-hidden bg-[#0a0a0a]">
            {/* Dynamic Ambient Background Light */}
            <div
              className="pointer-events-none absolute h-[800px] w-[800px] rounded-full opacity-20 blur-[120px] transition-all duration-700 ease-out"
              style={{
            ...
            <div className="relative flex aspect-square w-full max-w-md items-center justify-center md:aspect-[4/5]">
              {items.map((item, i) => {
                // Stack Logic
