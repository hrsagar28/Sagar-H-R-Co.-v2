import React from 'react';
import { useResourceData } from '../../hooks/useResourceData';
import Skeleton from '../../components/Skeleton';
import { AlertCircle } from 'lucide-react';

interface CIIRow {
  fy: string;
  index: string;
}

const CIITable: React.FC = () => {
  const { data: ciiData, loading, error } = useResourceData<CIIRow[]>('cii-data.json');

  return (
    <div className="rounded-[2.5rem] border border-brand-border bg-brand-surface p-8 shadow-sm md:p-12">
      <div className="mb-8">
        <h2 className="font-heading text-3xl font-bold text-brand-dark">Cost Inflation Index</h2>
        <p className="mt-1 text-brand-stone">For Long Term Capital Gains Calculation</p>
      </div>

      {loading ? (
        <div className="space-y-4">
          <Skeleton variant="rectangular" height={40} className="w-full rounded-lg" />
          {[1, 2, 3, 4, 5].map((i) => (
            <Skeleton key={i} variant="rectangular" height={50} className="w-full rounded-md" />
          ))}
        </div>
      ) : error ? (
        <div className="rounded-2xl border border-red-100 bg-red-50 p-8 text-center text-red-600">
          <AlertCircle className="mx-auto mb-2" size={24} />
          <p>{error}</p>
        </div>
      ) : (
        <div className="overflow-hidden rounded-2xl border border-brand-border">
          <table className="w-full text-left">
            <thead className="bg-brand-bg">
              <tr>
                <th className="px-8 py-4 text-sm font-bold uppercase tracking-widest text-brand-dark">
                  Financial Year
                </th>
                <th className="px-8 py-4 text-right text-sm font-bold uppercase tracking-widest text-brand-dark">
                  CII Index
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-brand-border/50">
              {ciiData &&
                ciiData.map((row, idx) => (
                  <tr key={idx} className="transition-colors hover:bg-brand-bg/30">
                    <td className="px-8 py-4 font-medium text-brand-stone">{row.fy}</td>
                    <td className="px-8 py-4 text-right font-mono text-lg font-bold text-brand-moss">{row.index}</td>
                  </tr>
                ))}
            </tbody>
          </table>
        </div>
      )}
      <p className="mt-6 text-center text-[10px] text-brand-stone">* Base Year 2001-02 = 100.</p>
    </div>
  );
};

export default CIITable;
